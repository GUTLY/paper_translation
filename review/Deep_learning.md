...
在20世纪90年代末期，神经网络和反向传播很大程度上被机器学习社区抛弃又被计算机视觉和语音识别社区忽视。人们普遍认为，从有限的先验知识（prior knowledge）学得一个有用的、多层的特征提取器是不可能的。特别的，通常认为，简单的梯度下降会陷入不良的局部极小值—权重配置，对其进行很小的变化将减少平均误差。 

在实践中，大型神经网络很少出现局部最小值。不管初始条件为何，系统几乎总是能得到相似质量的解。 最近的理论和实验结果有力地表明，局部极小值在一般情况下不是一个严重的问题。相反，取而代之的是，景观中包含大量的鞍点（saddle points），它的梯度为零，并且曲面在大多数维度（dimensions）上向上弯曲，而在其余维度上向下弯曲 **[29, 30]**。分析表明，鞍点只有在一些向下弯曲的方向（downward curving directions）有非常大的值，但是几乎所有这些鞍点的目标函数值都非常相似。因此，算法卡在这些鞍点中的哪一个上都没关系。

对深度前馈神经网络（deep feedforward networks）的研究兴趣在2006年被加拿大高级研究所（CIFAR）的一组研究员唤起 **[31-34]**。研究人员介绍到，无监督学习程序可以在无标签数据的情况下创建特征检测器层。学习特征检测器每一层的目的是能够在下一层中重建或建模特征检测器（或原始输入）的活动。通过使用此重建目标对多层逐渐复杂的特征检测器进行“预训练”，可以将深度网络的权重初始化为合理的值。最后一层的输出单元可以被添加到网络的顶部，整个深层系统可以使用标准的反向传播进行微调 **[33-35]**。这在识别手写数字或行人检测上有很好的效果，特别是当标签数据量非常有限时 **[36]**。

这种训练前方法的第一个主要应用是语音识别，便捷编程 **[37]**和使研究人员训练网络的速度提高了10到20倍的高速图形处理单元（GPU）的出现使之成为可能。在2009年，该方法用于将从声波提取的系数的短时间窗口映射到一组可能由窗口中心的帧表示的各种语音片段的概率。它在一个使用少量词汇量的标准语音识别基准测试中取得了破纪录的成绩
